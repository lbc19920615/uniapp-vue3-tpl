<template>
  ...
  <view
    v-if="inited"
    data-eventsync="true"
    :class="['zx-transition', customClass, classes]"
    :style="rootStyle"
...
function getClassNames(name: string | Array<string>) {
  ...
  if (Array.isArray(name)) {
    for (let index = 0; index < name.length; index++) {
      enter = `zx-${name[index]}-enter zx-${name[index]}-enter-active ${enter}`
      enterTo = `zx-${name[index]}-enter-to zx-${name[index]}-enter-active ${enterTo}`
      leave = `zx-${name[index]}-leave zx-${name[index]}-leave-active ${leave}`
      leaveTo = `zx-${name[index]}-leave-to zx-${name[index]}-leave-active ${leaveTo}`
    }
  } else {
    enter = `zx-${name}-enter zx-${name}-enter-active ${enter}`
    enterTo = `zx-${name}-enter-to zx-${name}-enter-active ${enterTo}`
    leave = `zx-${name}-leave zx-${name}-leave-active ${leave}`
    leaveTo = `zx-${name}-leave-to zx-${name}-leave-active ${leaveTo}`
  }
  return {
    enter: enter,
    'enter-to': enterTo,
...
.zx-zoom-out-enter,
.zx-zoom-out-leave-to {
  transform: scale(1.2);
  opacity: 0;
}

.zx-zoom-in-enter-active,
.zx-zoom-in-leave-active,
.zx-zoom-out-enter-active,
.zx-zoom-out-leave-active,
